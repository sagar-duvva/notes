...
$Subs = Get-AzSubscription | ? {$_.State -like "Enabled"}
...
Write-Host "Working on " $subname -ForegroundColor Green
Write-Host "Sub Id" $SubscriptionId -ForegroundColor Gray
###
###
if($subname -like '*Notes*' -and $subname -notlike '*QA*' -and $subname -notlike  "*Mover*" -and $subname -notlike '*Test*' ){
...
elseif($subname -like "*FileShare*" -and $subname -notlike "*QA*" -and $subname -notlike  "*Mover*" -and  $subname -notlike '*Test*'){
Select-AzSubscription -SubscriptionId $SubscriptionId | Out-Null
$VM = Get-AzVM
If($VM.Count -le 0){
$Inactive +=$subname
...
Write-Host "Data Collection Completed. Going to Execute CSV Merge Code" -ForegroundColor Yellow
...
$Date=Get-Date
Write-Host "Script Execution Completed $Date" -ForegroundColor Yellow
### End of the Script